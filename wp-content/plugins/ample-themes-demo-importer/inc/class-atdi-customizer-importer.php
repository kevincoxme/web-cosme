...
class ATDI_Customizer_Importer {
	...
	public static function import_customizer_options( $import_file_path ) {
		...
		if ( ! file_exists( $import_file_path ) ) {
			return new WP_Error(
				...
				sprintf(
					esc_html__( 'The customizer import file is missing! File path: %1$s', 'atdi' ),
					$import_file_path
				)
			);
		}
		...
		// Get the upload data.
		$raw  = ATDI_Helpers::data_from_file( $import_file_path );
		...
		if ( ! is_array( $data ) && ( ! isset( $data['template'] ) || ! isset( $data['mods'] ) ) ) {
			return new WP_Error(
				...
				esc_html__( 'The customizer import file is not in a correct format. Please make sure to use the correct customizer import file.', 'atdi' )
			);
		}
		if ( $data['template'] !== $template ) {
			return new WP_Error(
				'customizer_import_wrong_theme',
				esc_html__( 'The customizer import file is not suitable for current theme. You can only import customizer settings for the same theme or a child theme.', 'atdi' )
			);
		}

		// Import images.
		if ( apply_filters( 'atdi/customizer_import_images', true ) ) {
		...
		if ( isset( $data['options'] ) ) {
			...
			require ATDI_PATH . 'inc/class-atdi-customizer-option.php';

			foreach ( $data['options'] as $option_key => $option_value ) {
				$option = new ATDI_Customizer_Option( $wp_customize, $option_key, array(
					'default'    => '',
					'type'       => 'option',
